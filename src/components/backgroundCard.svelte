<script lang="ts">
  ...
  function showBack(i: string, d: string) {
    let ar = Array.prototype.slice.call(document.getElementsByClassName("fc-bg"))
    ar.forEach((e)=>{
...
>
  ...
  <ul class="w-[70%] pb-4 flex flex-col lg:grid lg:grid-cols-4 lg:w-[85%] gap-2">
    {#each items as item}
      <li
      ...
      >
        ...
        <div id={item.item+"-fc"} class="flip-card-inner fc-bg aspect-video lg:aspect-[3/5] ">
          <div class="  flip-card-front flex flex-row lg:flex-col gap-4 w-2/3 mx-auto items-center">
            <button
            class="w-full h-full"
            title={item.item}
          ...
          <div class="flip-card-back w-full h-full dark:border-slate-700">
            <div class="w-full flex mt-1">
              <button class="border border-slate-700 shadow-xl aspect-square h-fit w-8 rounded-full ml-2" on:click={() => document.getElementById(item.item+"-fc")?.classList.toggle("flip")}><i aria-hidden="true" class="fa-solid fa-chevron-left"></i><span class="sr-only">Back</span></button>

            </div>
              <!-- <h3 class="mt-2 self-start">{backTitle}</h3> -->

            <p class="my-3 mx-3 text-sm">{description}</p>
